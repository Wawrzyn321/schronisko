...
import { GALLERY_PATH, NEWS_IMAGES_PATH, TARGET_NEWS_PATH } from '../config';
...
function copyNewsImage() {
    if (!fs.existsSync(NEWS_IMAGES_PATH)) {
        throw Error("No source dir for news images")
    }
    if (!fs.existsSync(TARGET_NEWS_PATH)) {
        fs.mkdirSync(TARGET_NEWS_PATH, { recursive: true });
    }

    fse.copySync(NEWS_IMAGES_PATH, TARGET_NEWS_PATH);
}
...
export async function seedNews(prisma: PrismaClient, updateFiles: boolean) {
    ...
    for (const n of importedNews) {
        ...
        if (updateFiles) {
            if (n.descr.includes('<img')) {
                const r = /<img.*?src="(.*?)"/g;
                const imagePaths = [...n.descr.matchAll(r)].map(e => e[1]).filter(s => s.startsWith('gallery/'));
                for (const imagePath of imagePaths) {
                    const imageName = imagePath.replace('gallery/', '');
                    const fullImagePath = path.join(GALLERY_PATH, imageName);
                    if (fs.existsSync(fullImagePath)) {
                        console.log(path.join(TARGET_NEWS_PATH, imageName))
                        n.descr = n.descr.replace(imagePath, `news/${imageName}`);
                        fs.copyFileSync(fullImagePath, path.join(TARGET_NEWS_PATH, imageName))
                    }
                }
            }
        }
