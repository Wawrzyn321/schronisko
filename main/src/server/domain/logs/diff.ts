...
export function formattedDiff<T>(from: T, to: T, props: DiffProps<T>[], additionalChanges: Change[] = []): string {
...
export function havePermissionsChanged(prevPermissions: Permission[], newPermissions: Permission[]): boolean {
    return allPermissions.some(perm => prevPermissions.includes(perm) !== newPermissions.includes(perm));
}
