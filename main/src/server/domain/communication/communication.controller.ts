...
export class CommunicationController {
    ...
    @Post('/volunteer')
    sendVolunteering(@Query('id') id: string, @Query('text') text: string, @Body() props: VolunteeringFormFetch) {
        return this.communicationService.sendVolunteering(id, text, props);
    }

    @Post('/v-adoption')
    sendVAdoption(@Query('id') id: string, @Query('text') text: string, @Body() props: VAdoptionFormFetch) {
