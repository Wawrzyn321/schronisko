...
import { afterAdoptionQueryOptions, animalDetailsQueryOptions, animalImagesQueryOptions, animalsQueryOptions, newsQueryOptions, pageQueryOptions, recentNewsQueryOptions, settingsQueryOptions } from "./queryOptions";
import {
    AnimalCategory,
    AnimalType,
...
export function getAnimalListPageServerSideProps(pageId: string, args: AnimalListArgs) {
    return async function ({ query }: GetServerSidePropsContext) {
        const queryClient = new QueryClient();
        const queryParamPage = query.page as string;
        const initialPage = (Number.isNaN(parseInt(queryParamPage)) ? 1 : parseInt(queryParamPage)) - 1;

        await Promise.all([
            queryClient.prefetchQuery(pageQueryOptions(pageId)),
            queryClient.prefetchQuery(animalsQueryOptions({
                ...args,
...
export async function getGenericPageServerSideProps(context: GetServerSidePropsContext) {
    const queryClient = new QueryClient();
...
export function getGonePageServerSideProps(mainPageId: string, poemPageId: string) {
    return async function ({ query }: GetServerSidePropsContext) {
        const queryClient = new QueryClient();
        const queryParamPage = query.page as string;
        const initialPage = (Number.isNaN(parseInt(queryParamPage)) ? 1 : parseInt(queryParamPage)) - 1;
...
export async function getNewsServerSideOptions({ params }: GetServerSidePropsContext) {
    const queryClient = new QueryClient();
    const newsId = params.id as string;

    await queryClient.prefetchQuery(newsQueryOptions(newsId));
    return {
        props: {
            dehydratedState: dehydrate(queryClient),
            newsId
...
export async function getAnimalDetailsServerSideProps({ query }: GetServerSidePropsContext) {
    const queryClient = new QueryClient();
